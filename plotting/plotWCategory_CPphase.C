...
void plotWCategory_CPphase(){
    TFile *file = new TFile("workspaces/prod220222_1M_run230818_fix_pt20_eta40/jet_observables.root", "READ");
    ...
    // Branch names
    std::vector<TString> branchNames = {"wCategories"};
    ...
    TLegend *legend = new TLegend(0.15, 0.5, 0.45, 0.85);
    ...
    for (int iHist = 0; iHist < hists.size(); ++iHist) {
        ...
        hist->SetLineWidth(2);
        hist->GetYaxis()->SetTitle("Fraction of W bosons");
        ...
        h_ratio->GetXaxis()->SetTitle("W boson matching categories");
        ...
        h_ratio->GetYaxis()->SetRangeUser(0.69,1.21);
        const char* labels[] = {"contained qq", "resolved qq", "q", "none", "leptonic"};  // Extend as necessary
    ...
    // Save canvas
    canvas->SaveAs("plots/plot_230818_fix_CPphase_WCategory.pdf");
