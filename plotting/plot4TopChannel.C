...
void plot4TopChannel(){
    TFile *file = new TFile("workspaces/prod220222_1M_run230714/jet_observables.root", "READ");

    // Tree names
    std::vector<TString> treeNames = {"kt_1_alpha_0;1", "kt_1_alpha_45;1", "kt_1_alpha_90;1"};
    std::vector<TString> legendNames = {"SM Higgs (#alpha=0)", "Mixed Higgs (#alpha=#pi/4)", "CP-odd Higgs (#alpha=#pi/2)"}; 

    // Branch names
    std::vector<TString> branchNames = {"fourTopChannel"};
      
    // Color array
    const Int_t histColors[] = {1, 2, 4};
    ...
    TLegend *legend = new TLegend(0.55, 0.5, 0.85, 0.85);
    legend->SetTextFont(132);
    legend->SetHeader("Top-Yukawa coupling", "C");
    ...
    for (int iTree = 0; iTree < treeNames.size(); ++iTree) {
        //TFile *file = files[iFile];
        TString treeName = treeNames[iTree];

        TString histName = Form("%s_%s", treeName.Data(), branchName.Data());
        TH1D* hist = new TH1D(histName, histName, 6, -0.5, 5.5);  // Set the number of bins and range
    ...
    for (int iHist = 0; iHist < hists.size(); ++iHist) {
        ...
        const char *drawOp = (iHist == 0) ? "HIST TEXT00" : "SAME HIST";
        ...
        h_ratio->GetXaxis()->SetTitleSize(0.04/gPad->GetHNDC());
        h_ratio->GetXaxis()->SetLabelSize(0.05/gPad->GetHNDC());
        h_ratio->GetXaxis()->SetLabelOffset(0.03);
        h_ratio->GetXaxis()->SetNdivisions(5);
        h_ratio->GetXaxis()->SetTitle("Final state");
        h_ratio->GetYaxis()->SetTitle("Ratio to SM");
        h_ratio->GetYaxis()->SetTitleOffset(0.35);
        h_ratio->GetYaxis()->SetTitleSize(0.04/gPad->GetHNDC());
        h_ratio->GetYaxis()->SetLabelSize(0.04/gPad->GetHNDC());
        h_ratio->GetYaxis()->SetNdivisions(505);
        h_ratio->GetYaxis()->SetRangeUser(0.79,1.21);
        // Assuming h_ratio is your histogram
        h_ratio->GetXaxis()->SetBinLabel(1, "All had.");
        h_ratio->GetXaxis()->SetBinLabel(2, "1L");
        h_ratio->GetXaxis()->SetBinLabel(3, "2LOS");
        h_ratio->GetXaxis()->SetBinLabel(4, "2LSS");
        h_ratio->GetXaxis()->SetBinLabel(5, "3L");
        h_ratio->GetXaxis()->SetBinLabel(6, "4L");
        h_ratio->Draw(drawOp);

        // Ratio plot errors
        TGraphAsymmErrors *err_ratio = new TGraphAsymmErrors(h_ratio);
        err_ratio->SetFillColor(13);
    ...
    // Save canvas
    canvas->SaveAs("plots/plot_4TopChannel_CPphase.pdf");
