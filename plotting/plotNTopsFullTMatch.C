...
void plotNTopsFullTMatch(){
    TFile *fileNoEtaCut = new TFile("workspaces/prod220222_1M_run230712_etaNoCut/jet_observables.root", "READ");
    TFile *file40EtaCut = new TFile("workspaces/prod220222_1M_run230706_20GeV/jet_observables.root", "READ");
    TFile *file25EtaCut = new TFile("workspaces/prod220222_1M_run230712_eta2.5/jet_observables.root", "READ");
    vector<TFile*> files = {fileNoEtaCut, file40EtaCut, file25EtaCut};

    // Tree names
    std::vector<TString> treeNames = {"kt_1_alpha_0;1", "kt_1_alpha_45;1", "kt_1_alpha_90;1"};
    std::vector<TString> legendNames = {"No cut on #eta (#)", "#eta < 4.0", "#eta < 2.5"}; 

    // Branch names
    std::vector<TString> branchNames = {"numberOfTopsFullTMatch"};
      
    // Color array
    const Int_t histColors[] = {609, 617, 619};
    ...
    // Save canvas
    canvas->SaveAs("plots/plot_numberofTopsFullTMatch_etaCuts.pdf");
