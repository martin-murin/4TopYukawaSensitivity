...
void plotNTopsFullTMatch_CPphase(){
    TFile *file = new TFile("workspaces/prod220222_1M_run230816_pt20_eta40/jet_observables.root", "READ");
    ...
    // Branch names
    std::vector<TString> branchNames = {"numberOfLepTopsFullTMatch"};
    ...
    for (int iTree = 0; iTree < treeNames.size(); ++iTree) {
        ...
        TH1D* hist = new TH1D(histName, histName, 3, -0.5, 2.5);  // Set the number of bins and range
    ...
    for (int iHist = 0; iHist < hists.size(); ++iHist) {
        ...
        h_ratio->GetXaxis()->SetTitle("Number of fully matched leptonic tops");
    ...
    // Save canvas
    canvas->SaveAs("plots/plot_230816_CPphase_numberofLepTopsFullTMatch.pdf");
