...
void plotNTopsFullTMatch_leptonPTCut(){
    TFile *file20GeV = new TFile("workspaces/prod220222_1M_run230818_fix_pt20_eta40/jet_observables.root", "READ");
    TFile *file25GeV = new TFile("workspaces/prod220222_1M_run230818_fix_pt25_eta40/jet_observables.root", "READ");
    TFile *file30GeV = new TFile("workspaces/prod220222_1M_run230818_fix_pt30_eta40/jet_observables.root", "READ");
    vector<TFile*> files = {file20GeV, file25GeV, file30GeV};

    // Tree names
    std::vector<TString> treeNames = {"kt_1_alpha_0;1", "kt_1_alpha_45;1", "kt_1_alpha_90;1"};
    std::vector<TString> legendNames = {"#geq20 GeV (#)", "#geq25 GeV", "#geq30 GeV"}; 

    // Branch names
    std::vector<TString> branchNames = {"numberOfHadTopsFullTMatch"};
      
    // Color array
    const Int_t histColors[] = {880, 601, 810};
    ...
    TLegend *legend = new TLegend(0.15, 0.5, 0.4, 0.85);
    legend->SetTextFont(132);
    legend->SetHeader("Lepton p_{T} selection", "C");
    ...
    for (int iHist = 0; iHist < hists.size(); ++iHist) {
        ...
        h_ratio->GetXaxis()->SetTitle("Number of fully matched hadronic tops");
    ...
    // Save canvas
    canvas->SaveAs("plots/plot_230818_fix_leptonPTCut_numberofHadTopsFullTMatch.pdf");
