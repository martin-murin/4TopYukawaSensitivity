...
void plotWCategory(){
    ...
    vector<TFile*> files = {file20GeV, file30GeV, file20GeVEta25};

    // Tree names
    std::vector<TString> treeNames = {"kt_1_alpha_0;1", "kt_1_alpha_45;1", "kt_1_alpha_90;1"};
    std::vector<TString> legendNames = {"p_{T}^{lep} \\geq 20 GeV; |\\eta_{jet}| < 4.0 (#)", "p_{T}^{lep} \\geq 30 GeV; |\\eta_{jet}| < 4.0", "p_{T}^{lep} \\geq 20 GeV; |\\eta_{jet}| < 2.5"}; 
    ...
    TLegend *legend = new TLegend(0.15, 0.3, 0.55, 0.87);
    ...
    // Save canvas
    canvas->SaveAs("plots/plot_230818_fix_WCategory.pdf");
